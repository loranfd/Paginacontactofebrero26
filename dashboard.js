...
function procesarDatos(data) {
  ...
  const villasKeywords = ['promocion general', 'vivienda 03', 'vivienda 04', 'vivienda 05'];
  const villasIsla = data.filter(c => {
    const viviendaInteresada = normalizar(c['vivienda-interesada'] ?? '');
    return villasKeywords.some(k => viviendaInteresada.includes(k));
  });

  console.log('Villas detectadas:', villasIsla.length, 'Normal:', data.length - villasIsla.length);  // Debug counts
  ...
  // Ubicaciones: Parse avanzado - detect prov comunes, fallback Otros, unique sitios sorted
  const provinciasComunes = ['madrid', 'barcelona', 'valencia', 'sevilla', 'zaragoza', 'málaga', 'murcia', 'palma', 'las palmas', 'bilbao', 'alicante', 'córdoba', 'valladolid', 'vigo', 'gijón', 'hospitalet', 'vitoria', 'a coruña', 'granada', 'elche'];  // Añade más si sabes
  const ubicacionesPorProv = {};
  viviendaNormal.forEach(c => {
    let ubi = String(c['ubicacion-terreno'] ?? '').trim().toLowerCase();
    if (!ubi || ubi === 'sin especificar' || ubi === 'sin terreno') return;

    const parts = ubi.split(/[,;]/).map(p => p.trim()).filter(p => p);
    let provincia = 'Otros';
    for (let p of parts.reverse()) {  // Start from end
      if (provinciasComunes.includes(p) || p.length > 3 && !/[\d]/.test(p)) {
        provincia = p.charAt(0).toUpperCase() + p.slice(1);
        break;
      }
    }
    let sitio = parts.reverse().slice(0, -1).join(', ').trim() || ubi;

    if (!ubicacionesPorProv[provincia]) ubicacionesPorProv[provincia] = { count: 0, sitios: new Set() };
    ubicacionesPorProv[provincia].count++;
    if (sitio) ubicacionesPorProv[provincia].sitios.add(sitio);
  });
...
function calcularEmbudo() {
  ...
  // Palabras clave para identificar Villas
  const villasKeywords = ['promocion general', 'vivienda 03', 'vivienda 04', 'vivienda 05'];
  ...
  const contactosEnRango = datosGlobales.dataCompleta.filter(c => {
    ...
    // 3. Verificar que sea Villas
    const viviendaInteresada = normalizar(c['vivienda-interesada'] || '');
    const isVillas = villasKeywords.some(keyword => viviendaInteresada.includes(keyword));
